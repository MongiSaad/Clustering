...
for k in range(2, 10):
    ...
    db_idx = davies_bouldin_score(datanp, labels)
    stat.append([k, db_idx])
...
k_optimal = np.argmin(db_tab) + 2
plt.plot(k_tab, db_tab)
plt.xlabel("nb cluster")
plt.ylabel("Davies Bouldin Value")
plt.title("Davies Bouldin Method: k_optimal = "+ str(k_optimal))
plt.axvline(x=k_optimal, color="green")
plt.show()
